...
```{r data_manhattan}
manhattan <- read_csv("~/R/lecture-note-class-13-DharshanVaria/manhattan.csv")
```
...
- Population: 1 bedroom apartments
...
boot_prop <- mice %>% 
  filter(died == 0) %>% 
  mutate(treat = if_else(treat == 1, "yes", "no")) %>% 
  specify(response = treat, success = "yes") %>% 
  generate(reps = 10000, type = "bootstrap") %>% 
  calculate(stat = "prop")
```

```{r practice_2_ci}
ci_mice_lived <- get_ci(boot_prop, level = .95)
ci_mice_lived
```
...
The confidence is between .638 and .759.
